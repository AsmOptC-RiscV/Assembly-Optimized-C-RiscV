...
#define P12(s)                                                                 \
    ascon_inlined(s, 0xf0);                                                    \
    ascon_inlined(s, 0x96);
...
int crypto_aead_encrypt_opt_C6(unsigned char *c, unsigned long long *clen,
    ...
    *clen = mlen + CRYPTO_ABYTES;
    ...
    while (stage > 0) {
        if (from_len >= RATE) {
            ...
            s.x0 ^= BYTES_TO_U64(from, from_len);
            s.x0 ^= 0x80ull << (56 - 8 * from_len);
            stage -= 1;
            if (stage > 0) {
                from = m;
                from_len = mlen;
...
int crypto_aead_decrypt_opt_C6(unsigned char *m, unsigned long long *mlen,
